...
const remaining = (item: SrItem) => item.remaining_quantity ?? Math.max(Number(item.requested_quantity || 0) - Number(item.issued_quantity || 0), 0);
...
export default function SrPrintPreview({ srNumber, open, onClose, srData }: { srNumber: string; open: boolean; onClose: () => void; srData?: SrData | null }) {
  ...
  const downloadPdf = async () => {
    ...
    try {
      ...
      let y = M;
      const setFont = (bold=false, size=8, color: [number,number,number]=dark) => { pdf.setFont("helvetica", bold ? "bold" : "normal"); pdf.setFontSize(size); pdf.setTextColor(...color); };
      ...
      for (const row of metaRows) {
        ...
        for (let i=0;i<row.length;i+=2) {
          const lw=metaW[i], vw=metaW[i+1];
          setFont(true,6.2,dark); pdf.text(`${row[i]}:`,x,y+2,{baseline:"top"});
          const lines=wrap(String(row[i+1] || "-"),vw-1,6.7); setFont(false,6.7,dark); pdf.text(lines,x+lw,y+2,{baseline:"top",maxWidth:vw-1});
          x += lw+vw;
        }
        y += 8;
      }
      ...
      if (sr.request_details) {
        setFont(true,6.8,dark); pdf.text("Request Details:",M,y,{baseline:"top"});
        const lines=wrap(sr.request_details,contentW-27,6.8); setFont(false,6.8,dark); pdf.text(lines,M+27,y,{baseline:"top"});
        y += Math.max(6,lines.length*3)+3; rule(M,y,W-M,y,[82,82,82],0.25); y += 3;
      }

      const columns = [
        {title:"NO.",w:8,align:"center" as const},{title:"ITEM",w:42,align:"left" as const},{title:"SPECIFICATION",w:53,align:"left" as const},{title:"BRAND / MODEL",w:27,align:"left" as const},{title:"REQ.",w:14,align:"center" as const},{title:"STOCK",w:12,align:"center" as const},{title:"ISSUED",w:14,align:"center" as const},{title:"NOTE",w:20,align:"left" as const},
      ];
      const drawHeader=()=>{ const h=8; let x=M; for(const c of columns){box(x,y,c.w,h,soft);setFont(true,5.8,primary);pdf.text(c.title,c.align==="center"?x+c.w/2:x+2,y+2,{align:c.align,baseline:"top"});x+=c.w;} y+=h; };
      drawHeader();
      const drawRow=(item:SrItem,index:number)=>{
        const values=[String(index+1),`${item.item_name || "-"}\n${item.item_code || "-"}`,`${item.specification || item.item_type || "-"}${item.details ? `\n${item.details}` : ""}`,`${item.brand || "-"}${item.model ? `\n${item.model}` : ""}`,`${item.requested_quantity ?? 0} ${item.unit || ""}`.trim(),String(item.current_stock ?? "-"),String(item.issued_quantity ?? 0),`${remaining(item)}${item.item_note || item.note ? `\n${item.item_note || item.note}` : ""}`];
        const lh=3.1, sets=values.map((v,i)=>wrap(v,columns[i].w-4,6.6)), rh=Math.max(9,...sets.map(a=>a.length*lh+3.5));
        if(y+rh>H-42){pdf.addPage();y=M;drawHeader();}
        let x=M; sets.forEach((lines,i)=>{box(x,y,columns[i].w,rh);const tx=columns[i].align==="center"?x+columns[i].w/2:x+2;setFont(i===1,6.6,dark);pdf.text(lines,tx,y+2,{align:columns[i].align,baseline:"top"});if([1,2,3,7].includes(i)&&lines.length>1){setFont(false,5.5,muted);pdf.text(lines.slice(1),tx,y+2+lh,{align:columns[i].align,baseline:"top"});}x+=columns[i].w;}); y+=rh;
      };
      sr.items.forEach(drawRow);
      y += 4;
      const totalQty=sr.items.reduce((sum,item)=>sum+Number(item.requested_quantity||0),0), totalsW=42, totalsH=12;
      box(W-M-totalsW,y,totalsW,totalsH,undefined,[203,213,225]);setFont(false,6.2,dark);pdf.text(`Requested Items: ${sr.items.length}`,W-M-totalsW+3,y+2.5,{baseline:"top"});pdf.text(`Total Quantity: ${totalQty}`,W-M-totalsW+3,y+7,{baseline:"top"});y+=totalsH+6;
      ...
      const sigY=Math.min(y+22,H-42),gap=10,sigW=(contentW-gap)/2;
      rule(M,sigY,M+sigW,sigY,[75,85,99],0.35);rule(M+sigW+gap,sigY,W-M,sigY,[75,85,99],0.35);
      setFont(true,6.5,dark);pdf.text("Prepared By",M+sigW/2,sigY+2.5,{align:"center",baseline:"top"});pdf.text("Approved By",M+sigW+gap+sigW/2,sigY+2.5,{align:"center",baseline:"top"});
      setFont(true,7.5,dark);pdf.text(requesterName,M+sigW/2,sigY+7,{align:"center",baseline:"top"});pdf.text(approverName,M+sigW+gap+sigW/2,sigY+7,{align:"center",baseline:"top"});
      setFont(true,5.8,primary);pdf.text(`ID: ${requesterId}`,M+sigW/2,sigY+11.5,{align:"center",baseline:"top"});pdf.text(`ID: ${approverId}`,M+sigW+gap+sigW/2,sigY+11.5,{align:"center",baseline:"top"});
      setFont(false,5.3,muted);pdf.text(formatDate(sr.requested_at),M+sigW/2,sigY+15.5,{align:"center",baseline:"top"});pdf.text(formatDate(sr.processed_at),M+sigW+gap+sigW/2,sigY+15.5,{align:"center",baseline:"top"});
