...
export default function SrPrintPreview({ srNumber, open, onClose }: { srNumber: string; open: boolean; onClose: () => void }) {
  ...
  const printPage = () => {
    ...
    if (!win) return;
    win.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${sr.sr_number}</title><style>@page{size:A4 portrait;margin:0}*{box-sizing:border-box}html,body{margin:0;background:#fff}.sr-print-page{box-shadow:none!important;margin:0!important}</style></head><body>${pageRef.current.outerHTML}</body></html>`);
    win.document.close();
    win.focus();
  ...
  return (
    <div className="fixed inset-0 z-[100] flex flex-col bg-black/60 p-3 sm:p-5">
      <div className="mb-3 flex shrink-0 items-center justify-between rounded-xl border border-[var(--school-border)] bg-[var(--school-surface)] px-3 py-2.5 shadow-xl">
