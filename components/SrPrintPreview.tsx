  ...
  const downloadPdf = async () => {
    ...
    try {
      ...
      const drawRow=(item:SrItem,index:number)=>{
        const values=[String(index+1),`${item.item_name || "-"}\n${item.item_code || "-"}`,`${item.specification || item.item_type || "-"}${item.details ? `\n${item.details}` : ""}`,`${item.brand || "-"}${item.model ? `\n${item.model}` : ""}`,`${item.requested_quantity ?? 0} ${item.unit || ""}`.trim(),String(item.current_stock ?? "-"),String(item.issued_quantity ?? 0),String(remaining(item))];
        const lh=3.1;
        const sets=values.map((v,i)=>wrap(v,columns[i].w-4,6.6));
        const rh=Math.max(9,...sets.map(a=>a.length*lh+3.5));
        if(y+rh>H-42){pdf.addPage();y=M;drawHeader();}
        let x=M;
        sets.forEach((lines,i)=>{box(x,y,columns[i].w,rh);const tx=columns[i].align==="center"?x+columns[i].w/2:x+2;setFont(i===1,6.6,dark);pdf.text(lines,tx,y+2,{align:columns[i].align,baseline:"top",maxWidth:columns[i].w-4});x+=columns[i].w;});
        y+=rh;
      };
      ...
      setFont(true,7.5,dark);pdf.text(requesterName,M+sigW/2,sigY,{align:"center",baseline:"top"});pdf.text(approverName,M+sigW+gap+sigW/2,sigY,{align:"center",baseline:"top"});
      rule(M,sigY+5,M+sigW,sigY+5,[75,85,99],0.35);rule(M+sigW+gap,sigY+5,W-M,sigY+5,[75,85,99],0.35);
      setFont(true,6.5,dark);pdf.text("Prepared By",M+sigW/2,sigY+7.5,{align:"center",baseline:"top"});pdf.text("Approved By",M+sigW+gap+sigW/2,sigY+7.5,{align:"center",baseline:"top"});
      setFont(true,5.8,primary);pdf.text(`ID: ${requesterId}`,M+sigW/2,sigY+12,{align:"center",baseline:"top"});pdf.text(`ID: ${approverId}`,M+sigW+gap+sigW/2,sigY+12,{align:"center",baseline:"top"});
      setFont(false,5.3,muted);pdf.text(formatDate(sr.requested_at),M+sigW/2,sigY+16,{align:"center",baseline:"top"});pdf.text(formatDate(sr.processed_at),M+sigW+gap+sigW/2,sigY+16,{align:"center",baseline:"top"});
