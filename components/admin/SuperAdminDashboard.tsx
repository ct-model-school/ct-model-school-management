...
const modules: Module[] = [
  { id: "dashboard", title: "Dashboard", eyebrow: "Overview", description: "Live Super Admin overview, recent activity and pending actions.", stage: 1, sections: ["Overview"] },
...
export default function SuperAdminDashboard({ fullName, email, roleName, activityItems = [] }: { fullName?: string | null; email?: string | null; roleName: string; activityItems?: ActivityItem[] }) {
  ...
  const isDashboardOverview = selectedModule.id === "dashboard";
  const isParentSection = selectedModule.id === "parents";
  ...
  return <div className="mx-auto w-full max-w-[1550px]">
    <div className="grid min-h-[calc(100vh-7rem)] grid-cols-1 overflow-hidden rounded-[28px] border border-[var(--school-border)] bg-[var(--school-surface)] shadow-sm lg:grid-cols-[270px_minmax(0,1fr)]">
      <aside className="flex flex-col border-b border-[var(--school-border)] bg-[var(--school-surface)] lg:border-b-0 lg:border-r">
        <div className="border-b border-[var(--school-border)] px-5 py-6"><p className="text-[10px] font-black uppercase tracking-[0.18em] theme-primary">C.T. Model School</p><p className="mt-2 text-sm font-black text-[var(--school-text)]">Management System</p><div className="mt-4 flex items-center gap-2"><span className="rounded-full bg-[var(--school-primary-soft)] px-2.5 py-1 text-[9px] font-black uppercase tracking-wider theme-primary">Super Admin</span><StatusDot label="Active" /></div></div>
        <div className="p-4"><div className="mb-3 flex items-center justify-between px-2"><p className="text-[9px] font-black uppercase tracking-[0.18em] text-[var(--school-muted)]">Control Panel</p><span className="text-[9px] font-black theme-primary">{String(selectedModule.stage).padStart(2, "0")} / 07</span></div>
          <nav className="space-y-1" aria-label="Super Admin modules">{modules.map(module => { const active = selected === module.id; const expanded = openModule === module.id; return <div key={module.id}>
      ...
      <main className="min-w-0 bg-[var(--school-background)] p-4 sm:p-6 lg:p-8"><section className="min-h-[calc(100vh-11rem)] rounded-[26px] border border-[var(--school-border)] bg-[var(--school-surface)] p-5 sm:p-7 lg:p-8">
        <div className="flex flex-col gap-5 border-b border-[var(--school-border)] pb-6 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-[var(--school-primary-soft)] px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.16em] theme-primary">{selectedModule.eyebrow}</span><StatusDot label="System access active" /></div><p className="mt-5 text-[10px] font-black uppercase tracking-[0.18em] text-[var(--school-muted)]">C.T. Model School · Administration</p><h1 className="mt-2 text-3xl font-black tracking-tight text-[var(--school-text)] sm:text-4xl">{selectedSection}</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--school-muted)]">{isDashboardOverview ? `Welcome, ${displayName}. This is your live Super Admin control center.` : `${selectedModule.title} · ${selectedSection}`}</p></div><div className="grid shrink-0 grid-cols-2 gap-2 sm:w-[310px]"><div className="rounded-2xl border border-[var(--school-border)] bg-[var(--school-background)] p-3"><p className="text-[9px] font-black uppercase tracking-wider text-[var(--school-muted)]">Role</p><p className="mt-1 truncate text-xs font-black capitalize theme-primary">{roleLabel}</p></div><div className="rounded-2xl border border-[var(--school-border)] bg-[var(--school-background)] p-3"><p className="text-[9px] font-black uppercase tracking-wider text-[var(--school-muted)]">Module</p><p className="mt-1 truncate text-xs font-black text-[var(--school-text)]">{selectedModule.title}</p></div></div></div>

        {isDashboardOverview && <div className="mt-6 space-y-5">
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Stat label="Access" value="Active" note="Super Admin access" primary /><Stat label="Modules" value="07" note="Administration modules" /><Stat label="Recent Activity" value={String(activityItems.length)} note="Live records loaded" /><Stat label="Pending Actions" value={String(pendingItems.length)} note="Live records needing attention" /></div>
          <div className="grid gap-5 xl:grid-cols-[1.55fr_1fr]">
            <div className="rounded-2xl border border-[var(--school-border)] bg-[var(--school-background)] p-5 sm:p-6"><div className="flex items-end justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.16em] theme-primary">Live records</p><h2 className="mt-2 text-2xl font-black text-[var(--school-text)]">Recent System Activity</h2><p className="mt-1 text-sm text-[var(--school-muted)]">Existing school records, newest first.</p></div><span className="rounded-full border border-[var(--school-primary-border)] bg-[var(--school-surface)] px-3 py-1.5 text-[10px] font-black theme-primary">{activityItems.length}</span></div><div className="mt-5 overflow-hidden rounded-xl border border-[var(--school-border)] bg-[var(--school-surface)]">{activityItems.length ? <div className="divide-y divide-[var(--school-border)]">{activityItems.slice(0, 10).map(item => <div key={item.id} className="grid gap-2 px-4 py-3 sm:grid-cols-[1.35fr_0.8fr_0.75fr] sm:items-center"><div><p className="text-xs font-black text-[var(--school-text)]">{item.action}</p><p className="mt-0.5 truncate text-[10px] text-[var(--school-muted)]">{item.module}{item.detail ? ` · ${item.detail}` : ""}</p></div><p className="text-[10px] font-bold text-[var(--school-muted)]">{item.reference || "—"}</p><div className="flex items-center justify-between gap-2 sm:block"><span className="w-fit rounded-full border border-[var(--school-border)] bg-[var(--school-background)] px-2 py-1 text-[8px] font-black capitalize text-[var(--school-muted)]">{formatStatus(item.status)}</span><p className="mt-1 text-[9px] text-[var(--school-muted)]">{formatDate(item.createdAt)}</p></div></div>)}</div> : <Empty text="There are no recent records available to display." />}</div></div>
            <div className="rounded-2xl border border-[var(--school-border)] bg-[var(--school-background)] p-5 sm:p-6"><div className="flex items-end justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.16em] theme-primary">Live queue</p><h2 className="mt-2 text-2xl font-black text-[var(--school-text)]">Pending Actions</h2><p className="mt-1 text-sm text-[var(--school-muted)]">Only records needing attention.</p></div><span className="rounded-full border border-[var(--school-primary-border)] bg-[var(--school-surface)] px-3 py-1.5 text-[10px] font-black theme-primary">{pendingItems.length}</span></div><div className="mt-5 space-y-3">{pendingItems.length ? pendingItems.slice(0, 8).map(item => <div key={item.id} className="rounded-xl border border-[var(--school-border)] bg-[var(--school-surface)] p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-[9px] font-black uppercase tracking-[0.15em] theme-primary">{item.module}</p><h3 className="mt-1 truncate text-sm font-black text-[var(--school-text)]">{item.action}</h3><p className="mt-1 text-[10px] text-[var(--school-muted)]">{item.reference || "No reference"}</p></div><span className="shrink-0 rounded-full border border-[var(--school-border)] px-2 py-1 text-[8px] font-black capitalize text-[var(--school-muted)]">{formatStatus(item.status)}</span></div><a href={actionTarget(item)} className="mt-3 inline-flex rounded-lg border border-[var(--school-primary-border)] bg-[var(--school-primary-soft)] px-3 py-2 text-[9px] font-black theme-primary">Review →</a></div>) : <Empty text="No pending actions." />}</div></div>
          </div>
        </div>}

        {isParentSection && <SuperAdminParentWorkspace section={selectedSection as "Parent Accounts" | "Child Binding" | "Approvals" | "Parent Records"} />}

        {!isDashboardOverview && !isParentSection && <div className="mt-6 rounded-2xl border border-[var(--school-border)] bg-[var(--school-background)] p-6 sm:p-8"><p className="text-[10px] font-black uppercase tracking-[0.16em] theme-primary">{selectedModule.title}</p><h2 className="mt-2 text-2xl font-black text-[var(--school-text)]">{selectedSection}</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--school-muted)]">{selectedModule.description}</p><div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{selectedModule.sections.map(section => <button key={section} type="button" onClick={() => handleSectionClick(selectedModule, section)} className={`rounded-xl border px-4 py-3 text-left text-xs font-bold transition ${selectedSection === section ? "border-[var(--school-primary-border)] bg-[var(--school-primary-soft)] theme-primary" : "border-[var(--school-border)] bg-[var(--school-surface)] text-[var(--school-muted)] hover:bg-[var(--school-primary-soft)]"}`}>{section}</button>)}</div></div>}
      </section></main>
    </div>
  </div>;
}

function Stat({ label, value, note, primary = false }: { label: string; value: string; note: string; primary?: boolean }) {
  return <div className="rounded-2xl border border-[var(--school-border)] bg-[var(--school-background)] p-5"><p className="text-[9px] font-black uppercase tracking-wider text-[var(--school-muted)]">{label}</p><p className={`mt-2 truncate text-xl font-black ${primary ? "theme-primary" : "text-[var(--school-text)]"}`}>{value}</p><p className="mt-1 truncate text-xs text-[var(--school-muted)]">{note}</p></div>;
}
