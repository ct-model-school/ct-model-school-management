        ...
        <aside className="flex flex-col border-b border-[var(--school-border)] bg-[var(--school-surface)] lg:border-b-0 lg:border-r">
                    ...
                    {expanded && (
                      <div className="ml-11 mt-1 space-y-0.5 border-l border-[var(--school-border)] pl-3 pb-1">
                        {module.sections.map((section, index) => {
                          ...
                          return (
                            <button key={section} type="button" onClick={() => handleSectionClick(module, section)} className={`flex w-full items-center gap-2 rounded-xl px-2.5 py-2 text-left text-[10px] font-semibold transition ${sectionActive ? "bg-[var(--school-primary-soft)] font-black theme-primary" : "text-[var(--school-muted)] hover:bg-[var(--school-background)] hover:text-[var(--school-text)]"}`}>
          ...
          <div className="mt-auto border-t border-[var(--school-border)] p-4">
            <div className="rounded-2xl bg-[var(--school-primary-soft)] p-4">
              ...
              <button type="button" onClick={handleLogout} disabled={loggingOut} className="mt-4 w-full rounded-xl border border-[var(--school-primary-border)] bg-[var(--school-surface)] px-3 py-2 text-[10px] font-black theme-primary transition hover:opacity-80 disabled:cursor-wait disabled:opacity-60">{loggingOut ? "Logging out..." : "Logout"}</button>
            </div>
          </div>
        </aside>
            ...
            <div className="flex flex-col gap-5 border-b border-[var(--school-border)] pb-6 sm:flex-row sm:items-start sm:justify-between">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="rounded-full bg-[var(--school-primary-soft)] px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.16em] theme-primary">{selectedModule.eyebrow}</span>
                ...
                <h1 className="mt-2 text-3xl font-black tracking-tight text-[var(--school-text)] sm:text-4xl">{selectedSection}</h1>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--school-muted)]">{isDashboardOverview ? `Welcome, ${displayName}. This is your Super Admin control center.` : `${selectedModule.title} · ${selectedSection}`}</p>
              </div>
              <div className="grid shrink-0 grid-cols-2 gap-2 sm:w-[310px]">
                ...
                <div className="rounded-2xl border border-[var(--school-border)] bg-[var(--school-background)] p-3"><p className="text-[9px] font-black uppercase tracking-wider text-[var(--school-muted)]">Module</p><p className="mt-1 truncate text-xs font-black text-[var(--school-text)]">{selectedModule.title}</p></div>
              </div>
            </div>
            ...
            {isDashboardOverview ? (
              <div className="mt-6 space-y-5">
                <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                  <div className="rounded-2xl border border-[var(--school-border)] bg-[var(--school-background)] p-5"><p className="text-[9px] font-black uppercase tracking-wider text-[var(--school-muted)]">Access</p><p className="mt-2 text-xl font-black theme-primary">Active</p><p className="mt-1 text-xs text-[var(--school-muted)]">Super Admin access</p></div>
                  <div className="rounded-2xl border border-[var(--school-border)] bg-[var(--school-background)] p-5"><p className="text-[9px] font-black uppercase tracking-wider text-[var(--school-muted)]">Modules</p><p className="mt-2 text-xl font-black text-[var(--school-text)]">07</p><p className="mt-1 text-xs text-[var(--school-muted)]">Administration modules</p></div>
                  <div className="rounded-2xl border border-[var(--school-border)] bg-[var(--school-background)] p-5"><p className="text-[9px] font-black uppercase tracking-wider text-[var(--school-muted)]">Current</p><p className="mt-2 truncate text-xl font-black text-[var(--school-text)]">Dashboard</p><p className="mt-1 text-xs text-[var(--school-muted)]">Overview</p></div>
                ...
                <div className="rounded-2xl border border-[var(--school-border)] bg-[var(--school-primary-soft)] p-5 sm:p-6"><p className="text-[10px] font-black uppercase tracking-[0.16em] theme-primary">Quick access</p><h2 className="mt-2 text-xl font-black text-[var(--school-text)]">Select a module from the left</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--school-muted)]">Click any item in the left Control Panel. Its sections will open directly below it, and clicking a section will show that section here.</p></div>
              </div>
            ) : isSystemActivity ? (
              <div className="mt-6 space-y-5">
                <div className="rounded-2xl border border-[var(--school-border)] bg-[var(--school-primary-soft)] p-5 sm:p-6">
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                    <div><p className="text-[10px] font-black uppercase tracking-[0.16em] theme-primary">Live records</p><h2 className="mt-2 text-2xl font-black text-[var(--school-text)]">Recent System Activity</h2><p className="mt-1 text-sm text-[var(--school-muted)]">Recent activity from the existing school management records.</p></div>
                    <span className="rounded-full border border-[var(--school-primary-border)] bg-[var(--school-surface)] px-3 py-1.5 text-[10px] font-black theme-primary">{activityItems.length} recent</span>
                  </div>
                </div>
                {activityItems.length ? (
                  <div className="overflow-hidden rounded-2xl border border-[var(--school-border)]">
                    <div className="hidden grid-cols-[1.3fr_1fr_1fr_1fr] gap-4 bg-[var(--school-background)] px-5 py-3 text-[9px] font-black uppercase tracking-wider text-[var(--school-muted)] md:grid"><span>Activity</span><span>Reference</span><span>Status</span><span>Date & Time</span></div>
                    <div className="divide-y divide-[var(--school-border)]">
                      {activityItems.map((item) => (
                        <div key={item.id} className="grid gap-3 bg-[var(--school-surface)] px-5 py-4 md:grid-cols-[1.3fr_1fr_1fr_1fr] md:items-center md:gap-4">
                          <div><p className="text-xs font-black text-[var(--school-text)]">{item.action}</p><p className="mt-1 text-[10px] text-[var(--school-muted)]">{item.module}{item.detail ? ` · ${item.detail}` : ""}</p></div>
                          <p className="text-[10px] font-bold text-[var(--school-muted)]">{item.reference || "—"}</p>
                          <span className="w-fit rounded-full border border-[var(--school-border)] bg-[var(--school-background)] px-2.5 py-1 text-[9px] font-black capitalize text-[var(--school-muted)]">{formatStatus(item.status)}</span>
                          <p className="text-[10px] font-semibold text-[var(--school-muted)]">{formatDate(item.createdAt)}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                ) : (
                  <div className="rounded-2xl border border-dashed border-[var(--school-border)] bg-[var(--school-background)] p-10 text-center"><p className="text-sm font-black text-[var(--school-text)]">No recent activity</p><p className="mt-1 text-xs text-[var(--school-muted)]">There are no recent records available to display.</p></div>
                )}
              </div>
            ) : (
              <div className="mt-6 rounded-2xl border border-[var(--school-border)] bg-[var(--school-background)] p-6 sm:p-8">
                ...
                <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{selectedModule.sections.map((section) => <button key={section} type="button" onClick={() => handleSectionClick(selectedModule, section)} className={`rounded-xl border px-4 py-3 text-left text-xs font-bold transition ${selectedSection === section ? "border-[var(--school-primary-border)] bg-[var(--school-primary-soft)] theme-primary" : "border-[var(--school-border)] bg-[var(--school-surface)] text-[var(--school-muted)] hover:bg-[var(--school-primary-soft)] hover:text-[var(--school-text)]"}`}>{section}</button>)}</div>
              </div>
            )}
          </section>
        </main>
