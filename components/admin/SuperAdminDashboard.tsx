  ...
  return <div className="mx-auto w-full max-w-[1550px]">
    <div className="grid min-h-[calc(100vh-7rem)] grid-cols-1 overflow-hidden rounded-[28px] border border-[var(--school-border)] bg-[var(--school-surface)] shadow-sm lg:grid-cols-[270px_minmax(0,1fr)]">
      ...
      <main className="min-w-0 bg-[var(--school-background)] p-4 sm:p-6 lg:p-8"><section className="min-h-[calc(100vh-11rem)] rounded-[26px] border border-[var(--school-border)] bg-[var(--school-surface)] p-5 sm:p-7 lg:p-8">
        ...
        {isDashboardOverview && <div className="mt-6 space-y-5"><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Stat label="Access" value="Active" note="Super Admin access" primary /><Stat label="Modules" value="07" note="Administration modules" /><Stat label="Current" value="Dashboard" note="Overview" /><Stat label="Account" value={displayName} note={email || "Active account"} /></div><div className="rounded-2xl border border-[var(--school-border)] bg-[var(--school-primary-soft)] p-5 sm:p-6"><p className="text-[10px] font-black uppercase tracking-[0.16em] theme-primary">Quick access</p><h2 className="mt-2 text-xl font-black text-[var(--school-text)]">Select a module from the left</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--school-muted)]">Click any item in the left Control Panel. Its sections will open directly below it.</p></div></div>}

        {isSystemActivity && <div className="mt-6 space-y-5"><div className="rounded-2xl border border-[var(--school-border)] bg-[var(--school-primary-soft)] p-5 sm:p-6"><div className="flex items-end justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.16em] theme-primary">Live records</p><h2 className="mt-2 text-2xl font-black text-[var(--school-text)]">Recent System Activity</h2><p className="mt-1 text-sm text-[var(--school-muted)]">Recent activity from existing school management records.</p></div><span className="rounded-full border border-[var(--school-primary-border)] bg-[var(--school-surface)] px-3 py-1.5 text-[10px] font-black theme-primary">{activityItems.length} recent</span></div></div>{activityItems.length ? <div className="overflow-hidden rounded-2xl border border-[var(--school-border)]"><div className="hidden grid-cols-[1.3fr_1fr_1fr_1fr] gap-4 bg-[var(--school-background)] px-5 py-3 text-[9px] font-black uppercase tracking-wider text-[var(--school-muted)] md:grid"><span>Activity</span><span>Reference</span><span>Status</span><span>Date & Time</span></div><div className="divide-y divide-[var(--school-border)]">{activityItems.map(item => <div key={item.id} className="grid gap-3 bg-[var(--school-surface)] px-5 py-4 md:grid-cols-[1.3fr_1fr_1fr_1fr] md:items-center md:gap-4"><div><p className="text-xs font-black text-[var(--school-text)]">{item.action}</p><p className="mt-1 text-[10px] text-[var(--school-muted)]">{item.module}{item.detail ? ` · ${item.detail}` : ""}</p></div><p className="text-[10px] font-bold text-[var(--school-muted)]">{item.reference || "—"}</p><span className="w-fit rounded-full border border-[var(--school-border)] bg-[var(--school-background)] px-2.5 py-1 text-[9px] font-black capitalize text-[var(--school-muted)]">{formatStatus(item.status)}</span><p className="text-[10px] font-semibold text-[var(--school-muted)]">{formatDate(item.createdAt)}</p></div>)}</div></div> : <Empty text="There are no recent records available to display." />}</div>}

        {isPendingActions && <div className="mt-6 space-y-5"><div className="rounded-2xl border border-[var(--school-border)] bg-[var(--school-primary-soft)] p-5 sm:p-6"><div className="flex items-end justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.16em] theme-primary">Live records</p><h2 className="mt-2 text-2xl font-black text-[var(--school-text)]">Pending Actions</h2><p className="mt-1 text-sm text-[var(--school-muted)]">Only records that currently need attention are shown. No demo or placeholder entries.</p></div><span className="rounded-full border border-[var(--school-primary-border)] bg-[var(--school-surface)] px-3 py-1.5 text-[10px] font-black theme-primary">{pendingItems.length} pending</span></div></div>{pendingItems.length ? <div className="grid gap-4 lg:grid-cols-2">{pendingItems.map(item => <div key={item.id} className="rounded-2xl border border-[var(--school-border)] bg-[var(--school-background)] p-5"><p className="text-[9px] font-black uppercase tracking-[0.15em] theme-primary">{item.module}</p><h3 className="mt-2 text-lg font-black text-[var(--school-text)]">{item.action}</h3><p className="mt-3 text-xs text-[var(--school-muted)]">Reference: {item.reference || "—"}</p><p className="mt-1 text-xs text-[var(--school-muted)]">Created: {formatDate(item.createdAt)}</p><a href={actionTarget(item)} className="mt-5 inline-flex w-full items-center justify-center rounded-xl border border-[var(--school-primary-border)] bg-[var(--school-surface)] px-4 py-2.5 text-xs font-black theme-primary">Open {item.module} →</a></div>)}</div> : <Empty text="All currently loaded records are in a completed or non-actionable state." />}</div>}
