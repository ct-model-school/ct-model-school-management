...
const modules: Module[] = [
  { id: "dashboard", title: "Dashboard", eyebrow: "Overview", description: "Super Admin overview and rollout status.", stage: 1 },
  { id: "parents", title: "Parents & Guardians", eyebrow: "People", description: "Parent accounts, child binding, approvals and parent records.", stage: 2 },
  { id: "hr", title: "Human Resources", eyebrow: "HR", description: "Teacher and staff attendance, salary sheets and HR operations.", stage: 3 },
  { id: "inventory", title: "Inventory", eyebrow: "Store", description: "Items, stock, service requests and inventory operations.", stage: 4 },
  { id: "item-sr", title: "Item Service Request", eyebrow: "Workflow", description: "SR review, approval, issue and request history.", stage: 5 },
  { id: "accounts", title: "Accounts", eyebrow: "Finance", description: "Fees, payroll settlement, bills, vouchers and financial records.", stage: 6 },
  { id: "settings", title: "Access & Settings", eyebrow: "Control", description: "Roles, permissions, system settings and administrative controls.", stage: 7 },
];
      ...
      <div className="grid min-h-[calc(100vh-7rem)] grid-cols-1 overflow-hidden rounded-[28px] border border-[var(--school-border)] bg-[var(--school-surface)] shadow-sm lg:grid-cols-[270px_minmax(0,1fr)]">
        ...
        <aside className="border-b border-[var(--school-border)] bg-[var(--school-surface)] lg:border-b-0 lg:border-r">
          ...
          <div className="p-4">
            ...
            <nav className="space-y-1.5" aria-label="Super Admin modules">
              {modules.map(module => {
                ...
                return (
                  ...
                  >
                    ...
                    {locked ? (
                      <span className="rounded-full border border-[var(--school-border)] px-1.5 py-0.5 text-[8px] font-black uppercase text-[var(--school-muted)]">Next</span>
                    ) : (
                      <span className="h-1.5 w-1.5 rounded-full theme-primary-bg" />
                    )}
                  </button>
        ...
        <main className="min-w-0 bg-[var(--school-background)] p-4 sm:p-6 lg:p-8">
          {selectedModule.id === "dashboard" ? (
            <div className="space-y-5">
              ...
              <section className="rounded-[26px] border border-dashed border-[var(--school-border)] bg-[var(--school-surface)] p-5 sm:p-7">
                <p className="text-[10px] font-black uppercase tracking-[0.16em] theme-primary">Selected module</p>
                <h2 className="mt-2 text-2xl font-black text-[var(--school-text)]">Dashboard</h2>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--school-muted)]">All module navigation is now kept in the left slide panel. The right side is reserved only for the selected module view.</p>
              </section>
            </div>
          ) : (
            <section className="flex min-h-[calc(100vh-11rem)] items-center justify-center rounded-[26px] border border-[var(--school-border)] bg-[var(--school-surface)] p-6 text-center">
              <div className="max-w-md">
                ...
                <h2 className="mt-2 text-2xl font-black text-[var(--school-text)]">{selectedModule.title}</h2>
                <p className="mt-3 text-sm leading-6 text-[var(--school-muted)]">{selectedModule.description}</p>
