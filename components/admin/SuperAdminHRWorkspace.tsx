...
export default function SuperAdminHRWorkspace({ section }: { section: "Staff & Teachers" | "Attendance" | "Payroll" | "Monthly Salary Sheet" }) {
  ...
  if (section !== "Staff & Teachers") {
    const descriptions: Record<Exclude<typeof section, "Staff & Teachers">, string> = {
      Attendance: "Record and review Teacher and Staff attendance using the existing HR salary workflow.",
      Payroll: "Prepare payroll from live member salary and attendance data and send payable amounts to Accounts.",
      "Monthly Salary Sheet": "Prepare, revise and submit the monthly salary sheet from the live HR database."
    };
    return <div className="mt-5 space-y-5">
      <section className="rounded-3xl border border-[var(--school-border)] bg-[var(--school-surface)] p-5 shadow-sm sm:p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.16em] theme-primary">Human Resources</p>
            <h2 className="mt-1 text-2xl font-black text-[var(--school-text)]">{section}</h2>
            <p className="mt-1 max-w-3xl text-sm text-[var(--school-muted)]">{descriptions[section]}</p>
          </div>
          <Link href="/admin/members" className="w-fit rounded-xl theme-primary-bg px-4 py-2.5 text-xs font-black">Staff & Teachers →</Link>
        </div>
      </section>
      <HRPayrollWorkspace />
    </div>;
  ...
  return <div className="mt-5 space-y-5">
    <section className="rounded-3xl border border-[var(--school-border)] bg-[var(--school-surface)] p-5 shadow-sm sm:p-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <Link href="/admin/members" className="rounded-xl border border-[var(--school-primary-border)] bg-[var(--school-background)] px-4 py-2.5 text-[10px] font-black theme-primary">Manage Members</Link>
          <Link href="/admin/members?type=staff" className="rounded-xl theme-primary-bg px-4 py-2.5 text-[10px] font-black">+ Add Staff</Link>
          <Link href="/admin/members?type=teacher" className="rounded-xl theme-primary-bg px-4 py-2.5 text-[10px] font-black">+ Add Teacher</Link>
        </div>
      </div>
      <div className="mt-5 grid grid-cols-3 gap-2 sm:w-[360px]"><Stat label="Teachers" value={teachers} /><Stat label="Staff" value={staff} /><Stat label="Showing" value={filtered.length} primary /></div>
    ...
    <section className="overflow-hidden rounded-3xl border border-[var(--school-border)] bg-[var(--school-surface)] shadow-sm">
      <div className="flex flex-col gap-2 border-b border-[var(--school-border)] px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6"><div><p className="text-[10px] font-black uppercase tracking-[0.16em] theme-primary">Database records</p><p className="mt-1 text-xs text-[var(--school-muted)]">No demo or hardcoded member data.</p></div><Link href="/admin/members" className="text-[10px] font-black theme-primary">Open full member management →</Link></div>
      {error ? <div className="m-5 rounded-xl border border-red-200 bg-red-50 p-4 text-xs font-semibold text-red-700">{error}</div> : loading ? <div className="p-10 text-center text-sm text-[var(--school-muted)]">Loading live staff and teacher records...</div> : filtered.length === 0 ? <div className="p-10 text-center text-sm text-[var(--school-muted)]">No matching active staff or teacher records found.</div> : <div className="overflow-x-auto"><table className="min-w-[1080px] w-full"><thead><tr className="border-b border-[var(--school-border)] bg-[var(--school-background)] text-left text-[9px] font-black uppercase tracking-wider text-[var(--school-muted)]"><th className="px-5 py-3">ID</th><th className="px-5 py-3">Name</th><th className="px-5 py-3">Type</th><th className="px-5 py-3">Designation</th><th className="px-5 py-3">Department</th><th className="px-5 py-3">Subject</th><th className="px-5 py-3">Phone</th><th className="px-5 py-3">Status</th><th className="px-5 py-3">Action</th></tr></thead><tbody className="divide-y divide-[var(--school-border)]">{filtered.map(member => <tr key={`${member.member_type}-${member.member_id}`} className="hover:bg-[var(--school-background)]"><td className="px-5 py-4 text-xs font-black theme-primary">{member.member_id}</td><td className="px-5 py-4"><p className="text-xs font-black text-[var(--school-text)]">{member.full_name}</p><p className="mt-0.5 text-[10px] text-[var(--school-muted)]">{member.email || "No email"}</p></td><td className="px-5 py-4"><span className="rounded-full border border-[var(--school-border)] bg-[var(--school-background)] px-2.5 py-1 text-[9px] font-black capitalize text-[var(--school-muted)]">{member.member_type}</span></td><td className="px-5 py-4 text-xs text-[var(--school-muted)]">{member.designation || "—"}</td><td className="px-5 py-4 text-xs text-[var(--school-muted)]">{member.department || "—"}</td><td className="px-5 py-4 text-xs text-[var(--school-muted)]">{member.subject || "—"}</td><td className="px-5 py-4 text-xs text-[var(--school-muted)]">{member.phone || "—"}</td><td className="px-5 py-4"><span className="rounded-full border border-[var(--school-primary-border)] bg-[var(--school-primary-soft)] px-2.5 py-1 text-[9px] font-black theme-primary">Active</span></td><td className="px-5 py-4"><Link href={`/admin/members?edit=${encodeURIComponent(member.member_id)}`} className="rounded-lg border border-[var(--school-border)] bg-[var(--school-background)] px-3 py-2 text-[9px] font-black theme-primary">Edit</Link></td></tr>)}</tbody></table></div>}
    </section>
  </div>;
}
