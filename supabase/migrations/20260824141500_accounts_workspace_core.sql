...
create table if not exists public.accounts_account_heads (
  id uuid primary key default gen_random_uuid(), head_type text not null, head_name text not null,
...
begin
  v_user := public.store_get_current_user(p_token); v_accounts := v_user->'permissions'->'accounts';
  return coalesce((v_accounts->>p_key)::boolean,false) or case when jsonb_typeof(v_accounts)='boolean' then (v_accounts #>> '{}')::boolean else false end;
...
begin
  ...
  into v_count,v_income,v_expense,v_cash,v_bank from public.accounts_entries;
...
begin
  ...
  return query select e.id,e.voucher_no,e.entry_date,e.entry_type,e.category,e.party_type,e.party_id,e.party_name,e.amount,e.payment_method,e.account_name,e.reference_no,e.description,e.status,e.created_at
...
begin
  ...
  return query select t.member_id,'teacher',t.full_name,t.designation,t.department,t.salary from public.teacher_members t where t.is_active=true
