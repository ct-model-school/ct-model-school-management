...
begin
  ...
  if v_is_new and v_password is null then raise exception 'Password is required for a new member'; end if;
  ...
  if v_type = 'staff' then
    if v_is_new then
      ...
      select member_id into v_member_id from public.staff_members where id=p_id for update;
      if v_member_id is null then raise exception 'Staff member not found'; end if;
      update public.staff_members set full_name=trim(p_full_name),password_hash=coalesce(case when v_password is not null then crypt(v_password,gen_salt('bf')) end,password_hash),designation=nullif(trim(p_designation),''),department=nullif(trim(p_department),''),phone=nullif(trim(p_phone),''),email=nullif(trim(p_email),''),details=nullif(trim(p_details),''),access_role=v_role,updated_at=now() where id=p_id;
    end if;
    ...
    if v_is_new then
      ...
      select member_id into v_member_id from public.teacher_members where id=p_id for update;
      if v_member_id is null then raise exception 'Teacher member not found'; end if;
      update public.teacher_members set full_name=trim(p_full_name),password_hash=coalesce(case when v_password is not null then crypt(v_password,gen_salt('bf')) end,password_hash),designation=nullif(trim(p_designation),''),department=nullif(trim(p_department),''),subject=nullif(trim(p_subject),''),qualification=nullif(trim(p_qualification),''),phone=nullif(trim(p_phone),''),email=nullif(trim(p_email),''),details=nullif(trim(p_details),''),access_role=v_role,updated_at=now() where id=p_id;
    end if;
    ...
    if v_is_new then
      ...
      select member_id into v_member_id from public.account_members where id=p_id for update;
      if v_member_id is null then raise exception 'Accounts member not found'; end if;
      update public.account_members set full_name=trim(p_full_name),password_hash=coalesce(case when v_password is not null then crypt(v_password,gen_salt('bf')) end,password_hash),designation=nullif(trim(p_designation),''),department=nullif(trim(p_department),''),account_role=nullif(trim(p_account_role),''),phone=nullif(trim(p_phone),''),email=nullif(trim(p_email),''),details=nullif(trim(p_details),''),access_role=v_role,updated_at=now() where id=p_id;
    end if;
  else
    if v_is_new then
      ...
      select member_id into v_member_id from public.other_members where id=p_id for update;
      if v_member_id is null then raise exception 'Other member not found'; end if;
      update public.other_members set full_name=trim(p_full_name),password_hash=coalesce(case when v_password is not null then crypt(v_password,gen_salt('bf')) end,password_hash),designation=nullif(trim(p_designation),''),department=nullif(trim(p_department),''),role_title=nullif(trim(p_role_title),''),phone=nullif(trim(p_phone),''),email=nullif(trim(p_email),''),details=nullif(trim(p_details),''),access_role=v_role,updated_at=now() where id=p_id;
    end if;
  end if;
  ...
  return jsonb_build_object('id',p_id,'member_id',v_member_id,'member_type',v_type,'login_id',v_member_id,'access_role',v_role,'password_issued',v_password is not null,'password',v_password);
end;
$$;
...
create or replace function public.store_admin_reset_member_password(p_member_type text,p_id uuid)
returns jsonb
...
begin
  if not public.store_is_admin() then raise exception 'Not authorized'; end if;
  if v_type='staff' then update public.staff_members set password_hash=v_hash,updated_at=now(),is_active=true where id=p_id returning member_id into v_member_id;
  elsif v_type='teacher' then update public.teacher_members set password_hash=v_hash,updated_at=now(),is_active=true where id=p_id returning member_id into v_member_id;
  elsif v_type='accounts' then update public.account_members set password_hash=v_hash,updated_at=now(),is_active=true where id=p_id returning member_id into v_member_id;
