...
type ModuleKey = "dashboard" | "profile" | "item_sr" | "attendance" | "students" | "parents" | "people" | "teachers" | "store_members" | "inventory" | "accounts" | "notices" | "results";
type ModuleDefinition = { key: ModuleKey; label: string };
...
const modules: ModuleDefinition[] = [
  ...
  { key: "people", label: "People & Achievements" },
  { key: "teachers", label: "Teachers & Staff" },
  { key: "store_members", label: "Store Members" },
...
const permissionKey: Partial<Record<ModuleKey, string>> = {
...
export default function MemberDashboardPage() {
  ...
  useEffect(() => {
    ...
    async function loadUser() {
      ...
      if (!token) { router.replace("/loginportal"); return; }
  ...
  const canAccess = (key: ModuleKey) => {
    ...
    if (key === "item_sr") return itemSrAllowed;
    if (key === "accounts") return accountsAllowed;
    if (key === "dashboard" || key === "profile") return Boolean(user.permissions?.[key]);
    const keyName = permissionKey[key];
    return keyName ? Boolean(user.permissions?.[keyName]) : false;
  };
  ...
  return <main className="min-h-screen bg-[var(--school-background)] px-3 py-4 sm:px-5 sm:py-6 lg:px-8"><div className="mx-auto max-w-6xl">
    ...
    <section className="mt-5 rounded-3xl border border-[var(--school-border)] bg-[var(--school-surface)] p-5 shadow-sm sm:p-7">
      ...
      {effectiveActiveModule !== "dashboard" && effectiveActiveModule !== "profile" && effectiveActiveModule !== "item_sr" && effectiveActiveModule !== "inventory" && effectiveActiveModule !== "accounts" && <div className="mt-6 rounded-2xl border border-dashed border-[var(--school-border)] bg-[var(--school-primary-soft)] p-8 text-center"><h3 className="text-base font-black text-[var(--school-text)]">{activeLabel}</h3><p className="mt-2 text-sm text-[var(--school-muted)]">Access is enabled for your role. This section is ready for its module features.</p></div>}
    </section>
  </div></main>;
}
