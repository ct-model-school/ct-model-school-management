...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-[var(--school-background)] text-[var(--school-text)]">
      ...
      <section className="border-b border-[var(--school-border)] bg-[var(--school-surface)]"><div className="mx-auto grid max-w-7xl gap-px bg-[var(--school-border)] sm:grid-cols-2 lg:grid-cols-4">{[["Established", settings.established_year ? String(settings.established_year) : "2010"],["Board", settings.board || "Bangladesh"],["School", settings.school_short_name || "CTMS"],["Location", settings.address || DEFAULT_SETTINGS.address]].map(([label,value]) => <div key={label} className="min-w-0 overflow-hidden bg-[var(--school-surface)] px-4 py-5 sm:px-6 sm:py-7"><p className="text-[10px] font-extrabold uppercase tracking-[0.14em] theme-primary sm:text-xs">{label}</p><p className="mt-2 break-words text-base font-black leading-6 sm:text-xl sm:leading-7">{value}</p></div>)}</div></section>
