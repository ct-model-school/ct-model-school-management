...
export default function Home() {
  ...
  return (<main className="min-h-screen bg-[var(--school-background)] text-[var(--school-text)]">
    ...
    {people.length > 0 ? <section className="px-4 py-14 sm:px-6 lg:px-8"><div className="mx-auto max-w-7xl rounded-[2rem] border border-[var(--school-border)] bg-[var(--school-surface)] p-5 shadow-sm sm:p-7"><div className="mb-7 flex items-end justify-between gap-4"><div><p className="text-xs font-extrabold uppercase tracking-[0.18em] theme-primary">PEOPLE & ACHIEVEMENTS</p><h2 className="mt-2 text-3xl font-black tracking-tight sm:text-4xl">Our School Community</h2><p className="mt-3 text-sm leading-6 text-[var(--school-muted)] sm:text-base">Meet the teachers, committee members, staff and students who make C.T. Model School special.</p></div><a href="/people" className="hidden shrink-0 rounded-xl border border-[var(--school-primary-border)] px-4 py-2 text-xs font-extrabold theme-primary transition hover:-translate-y-0.5 sm:inline-flex">Explore Community <span className="ml-1">→</span></a></div><div className="grid grid-cols-2 gap-3 sm:gap-5 md:grid-cols-3 lg:grid-cols-5">{people.map((person) => { const isStudent = person.category === "gpa5" || person.category === "scholarship";
...
})}</div><div className="mt-6 text-center sm:hidden"><a href="/people" className="text-sm font-bold theme-primary hover:underline">View all Community & achievements →</a></div></div></section> : null}
