...
function normalizeHomeLink(value: string | null | undefined, fallback: string) {
  const link = clean(value ?? null);
  if (!link) return fallback;
  if (link === "/about" || link === "/about/" || link.endsWith("/about") || link.endsWith("/about/")) return "#about";
  if (link === "/contact" || link === "/contact/" || link.endsWith("/contact") || link.endsWith("/contact/")) return "#contact";
  return link;
}
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    async function loadHome() {
      const [settingsResult, slidesResult, peopleResult] = await Promise.all([
        ...
        supabase.from("people_profiles").select("id,category,full_name,photo_url,designation,committee_position,subject,short_description,email,phone,whatsapp").eq("is_active", true).order("display_order", { ascending: true }).order("full_name", { ascending: true }).limit(5),
      ]);
      if (!mounted) return;
  ...
  const heroButton1Link = normalizeHomeLink(settings.hero_button_1_link, "#about");
  const heroButton2Link = normalizeHomeLink(settings.hero_button_2_link, "#contact");
    ...
    <main className="min-h-screen bg-[var(--school-background)] text-[var(--school-text)]">
      <header className="sticky top-0 z-50 border-b border-[var(--school-border)] bg-[var(--school-surface)]/95 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-3 lg:px-8">
          <a href="#top" className="flex min-w-0 items-center gap-3">{logoUrl ? <img src={logoUrl} alt={`${schoolName} logo`} className="h-12 w-16 rounded-lg object-contain" /> : <span className="theme-primary-bg flex h-11 w-11 items-center justify-center rounded-xl text-sm font-extrabold">CT</span>}<span className="min-w-0"><span className="block whitespace-nowrap text-xl font-black leading-tight sm:text-2xl lg:text-3xl">{schoolName}</span></span></a>
          <nav className="hidden items-center gap-7 text-sm font-semibold lg:flex"><a href="#top" className="theme-primary">Home</a><a href="#about">About</a><a href="#principal">Principal</a><a href="#contact">Contact</a></nav>
      ...
      {settings.show_hero ? <>
        <section className="relative w-full overflow-hidden bg-[var(--school-text)]"><div className="relative w-full overflow-hidden">{heroImages.length ? <img src={heroImages[0].image_url} alt="" aria-hidden="true" className="block h-auto w-full select-none opacity-0" /> : <div className="aspect-video w-full theme-primary-bg" /> : null}{heroImages.map((slide, index) => <div key={slide.id} className={`absolute inset-0 h-full w-full overflow-hidden transition-opacity ${index === activeSlide ? "opacity-100" : "pointer-events-none opacity-0"}`} style={{ transitionDuration: `${Math.max(200, settings.hero_transition_speed || 600)}ms` }}><img src={slide.image_url} alt={String(slide.alt_text || schoolName)} className="block h-full w-full object-cover object-center" /></div>)}{heroImages.length > 1 ? <div className="absolute bottom-6 left-1/2 z-10 flex -translate-x-1/2 gap-2">{heroImages.map((slide, index) => <button key={slide.id} type="button" onClick={() => setActiveSlide(index)} className={`h-2.5 rounded-full transition-all ${index === activeSlide ? "theme-primary-bg w-9" : "w-2.5 bg-white/70"}`} aria-label={`Show hero slide ${index + 1}`} />)}</div> : null}</div></section>
        <section className="border-b border-[var(--school-border)] bg-[var(--school-surface)]"><div className="mx-auto max-w-7xl px-5 py-12 lg:px-8 lg:py-14"><div className="max-w-4xl"><p className="text-xs font-extrabold uppercase tracking-[0.18em] theme-primary">{settings.hero_badge || DEFAULT_SETTINGS.hero_badge}</p><h1 className="mt-3 text-3xl font-black tracking-tight sm:text-4xl lg:text-5xl">{heroTitle}</h1>{settings.hero_subtitle ? <p className="mt-4 text-xl font-semibold text-[var(--school-muted)] sm:text-2xl">{settings.hero_subtitle}</p> : null}<p className="mt-5 max-w-3xl text-base leading-8 text-[var(--school-muted)] sm:text-lg">{heroDescription}</p><div className="mt-7 flex flex-wrap gap-3"><a href={heroButton1Link} className="theme-primary-bg rounded-xl px-5 py-3 text-sm font-extrabold text-white">{settings.hero_button_1_text || "Explore the School"}</a><a href={heroButton2Link} className="rounded-xl border border-[var(--school-primary-border)] px-5 py-3 text-sm font-extrabold">{settings.hero_button_2_text || "Contact Us"}</a></div></div></div></section>
      </> : null}
      ...
      <footer className="border-t border-[var(--school-border)] bg-[var(--school-surface)] px-5 py-8"><div className="mx-auto flex max-w-7xl flex-col gap-3 text-sm sm:flex-row sm:items-center sm:justify-between"><div><p className="font-bold">{schoolName}</p><p className="text-xs text-[var(--school-muted)]">{settings.address || DEFAULT_SETTINGS.address}</p></div><p className="text-xs text-[var(--school-muted)]">© {new Date().getFullYear()} {schoolName}. All rights reserved.</p></div></footer>
    </main>
  );
}
