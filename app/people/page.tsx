...
const categoryStyle = {
  backgroundColor: "color-mix(in srgb, var(--school-primary) 8%, var(--school-surface))",
          ...
          {categories.map((category) => {
            ...
            return (
              <section key={category.value} id={category.value} className="scroll-mt-24">
                <div className="mb-6 flex items-center gap-4 rounded-2xl border bg-[var(--school-surface)] px-5 py-4 shadow-sm" style={categoryStyle}>
                ...
                <div className="grid grid-cols-2 gap-4 sm:gap-5 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
                  {items.map((item) => {
                    ...
                    return (
                      <article key={item.id} className="group relative flex min-h-full flex-col overflow-hidden rounded-2xl border border-[var(--school-border)] bg-[var(--school-surface)] shadow-[0_4px_14px_rgba(0,0,0,0.06)] transition duration-300 hover:-translate-y-1.5 hover:border-[var(--school-primary-border)] hover:shadow-[0_14px_34px_rgba(0,0,0,0.12)]">
                        <div className="absolute inset-x-0 top-0 z-10 h-1 theme-primary-bg" />
                        ...
                        <div className="flex flex-1 flex-col px-3.5 py-4 sm:px-4 sm:py-4.5">
                          ...
                          {hasDetails ? (
                            <details className="mt-3">
                              <summary className="cursor-pointer list-none text-[10px] font-extrabold theme-primary hover:underline sm:text-xs">Read more</summary>
                              <div className="mt-2.5 space-y-1.5 rounded-xl border border-[var(--school-border)] bg-[var(--school-background)] p-2.5 text-[10px] leading-4 sm:text-xs sm:leading-5">
                          ...
                          ) : <span className="mt-3 text-[10px] font-extrabold theme-primary sm:text-xs">Read more</span>}

                          {(item.email || item.phone || item.whatsapp) ? (
                            <div className="mt-auto flex items-center justify-center gap-3 border-t border-[var(--school-border)] pt-4 sm:gap-4">
                              {item.email ? <a href={`mailto:${item.email}`} aria-label={`Email ${item.full_name}`} title={item.email} className="flex h-9 w-9 items-center justify-center rounded-xl bg-[var(--school-primary-soft)] text-[var(--school-primary)] transition hover:scale-110"><Icon type="mail" /></a> : null}
                              {item.phone ? <a href={`tel:${item.phone}`} aria-label={`Call ${item.full_name}`} title={item.phone} className="flex h-9 w-9 items-center justify-center rounded-xl bg-orange-50 text-orange-500 transition hover:scale-110"><Icon type="phone" /></a> : null}
                              {item.whatsapp ? <a href={whatsappUrl(item.whatsapp)} target="_blank" rel="noreferrer" aria-label={`WhatsApp ${item.full_name}`} title={item.whatsapp} className="flex h-9 w-9 items-center justify-center rounded-xl bg-green-50 text-green-600 transition hover:scale-110"><Icon type="whatsapp" /></a> : null}
                            </div>
                          ) : null}
                        </div>
                      </article>
