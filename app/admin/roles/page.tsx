...
const accountsList = [["salary_view", "View Salary Sheets"], ["salary_status", "View Salary Status"], ["salary_approval", "Approve Salary"], ["salary_payment", "Make Salary Payment"], ["salary_history", "View Salary History"]] as const;
...
export default function AdminRolesPage() {
  ...
  const [accounts, setAccounts] = useState<BoolMap>(() => empty(accountsList.map(x => x[0])));
  ...
  function reset() {
    ...
    setAccounts(empty(accountsList.map(x => x[0])));
  ...
  function editRole(role: Role) {
    ...
    setAccounts({ ...empty(accountsList.map(x => x[0])), ...acc });
  ...
  function toggleAll(value: boolean) {
    ...
    setAccounts(Object.fromEntries(accountsList.map(([key]) => [key, value])));
  ...
  return <AdminPageShell eyebrow="Access Control" title="Role Management" description="Every ready category is independent. Admin can enable or disable each permission for each role without duplicate access names.">
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1.08fr)_minmax(360px,.92fr)]">
      <section className="rounded-3xl border border-[var(--school-border)] bg-[var(--school-surface)] p-5 shadow-sm sm:p-6">
        ...
        {rows("Accounts • Salary", "Salary access is separated so each role can receive only the required capability.", accountsList, accounts, setAccounts, `${Object.values(accounts).filter(Boolean).length}/5`)}
      ...
      <section className="rounded-3xl border border-[var(--school-border)] bg-[var(--school-surface)] p-5 shadow-sm sm:p-6"><p className="text-[10px] font-black uppercase tracking-[0.16em] theme-primary">Role Library</p><h2 className="mt-1 text-xl font-black">Available Roles</h2><p className="mt-1 text-xs leading-5 text-[var(--school-muted)]">Salary permissions are shown individually for quick auditing.</p><div className="mt-5 space-y-3">{loading ? <p className="rounded-2xl border border-dashed border-[var(--school-border)] p-8 text-center text-sm text-[var(--school-muted)]">Loading roles...</p> : null}{!loading && roles.map(role => { const acc = role.permissions?.accounts && typeof role.permissions.accounts === "object" ? role.permissions.accounts as BoolMap : {}; const enabled = accountsList.filter(([key]) => Boolean(acc[key])); return <article key={role.id} className="rounded-2xl border border-[var(--school-border)] p-4"><div className="flex items-start justify-between gap-3"><div><h3 className="text-sm font-black">{role.role_name}</h3><p className="mt-1 text-[10px] text-[var(--school-muted)]">Salary permissions: {enabled.length}/5</p></div><div className="flex gap-2"><button type="button" onClick={() => editRole(role)} className="rounded-xl border border-[var(--school-border)] px-3 py-2 text-xs font-bold">Edit</button>{!role.is_system ? <button type="button" onClick={() => void remove(role)} className="rounded-xl border border-red-200 px-3 py-2 text-xs font-bold text-red-600">Remove</button> : null}</div></div>{enabled.length>0 ? <div className="mt-3 flex flex-wrap gap-2">{enabled.map(([key,label])=><span key={key} className="rounded-lg bg-[var(--school-primary-soft)] px-2 py-1 text-[10px] font-semibold theme-primary">{label}</span>)}</div> : null}</article>; })}</div></section>
    </div>
  </AdminPageShell>;
}
