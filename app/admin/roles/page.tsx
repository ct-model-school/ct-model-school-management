...
const inventoryPermissionList: { key: keyof InventoryPermissions; label: string; description: string }[] = [
  ...
  { key: "sr_approval", label: "SR Approval", description: "Review, approve, reject and process Item SR requests" },
];

const itemSrPermissionList: { key: keyof ItemSrPermissions; label: string; description: string }[] = [
  { key: "view", label: "View / Search Items", description: "Search available items by code, name, type or specification" },
  { key: "create", label: "Create SR", description: "Select items, enter quantity and request details, then submit an Item Service Request" },
  { key: "history", label: "View Own SR History", description: "View submitted SRs and their current approval or issue status" },
];
...
export default function AdminRolesPage() {
  ...
  function resetForm() {
    setEditingId(null); setRoleName(""); setInventory(emptyInventory()); setItemSr(emptyItemSr()); setLegacy(emptyLegacy());
  }
  ...
  return <AdminPageShell eyebrow="Access Control" title="Role Management" description="Build each role from independent permissions. Inventory and Item SR can be enabled or disabled separately for every role.">
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1.08fr)_minmax(360px,.92fr)]">
      ...
      <section className="rounded-3xl border border-[var(--school-border)] bg-[var(--school-surface)] p-5 shadow-sm sm:p-6"><div><p className="text-[10px] font-black uppercase tracking-[0.16em] theme-primary">Role Library</p><h2 className="mt-1 text-xl font-black">Available Roles</h2><p className="mt-1 text-xs leading-5 text-[var(--school-muted)]">Each role has independent Inventory, Item SR and Other permission sets.</p></div><div className="mt-5 space-y-3">{loading ? <p className="rounded-2xl border border-dashed border-[var(--school-border)] p-8 text-center text-sm text-[var(--school-muted)]">Loading roles...</p> : null}{!loading && roles.map((role) => { const inv = role.permissions?.inventory && typeof role.permissions.inventory === "object" ? role.permissions.inventory as Partial<InventoryPermissions> : {}; const sr = role.permissions?.item_sr && typeof role.permissions.item_sr === "object" ? role.permissions.item_sr as Partial<ItemSrPermissions> : {}; const invCount = inventoryPermissionList.filter((item) => Boolean(inv[item.key])).length; const srCount = itemSrPermissionList.filter((item) => Boolean(sr[item.key])).length; const legacyCount = legacyModules.filter((item) => Boolean(role.permissions?.[item.key])).length; return <article key={role.id} className="rounded-2xl border border-[var(--school-border)] p-4"><div className="flex items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><h3 className="text-sm font-black">{role.role_name}</h3>{role.is_system ? <span className="rounded-full bg-[var(--school-primary-soft)] px-2 py-1 text-[9px] font-black theme-primary">SYSTEM</span> : null}</div><p className="mt-1 text-[10px] text-[var(--school-muted)]">Inventory {invCount}/5 · Item SR {srCount}/3 · Other {legacyCount}</p></div><div className="flex gap-2"><button type="button" onClick={() => editRole(role)} className="rounded-xl border border-[var(--school-border)] px-3 py-2 text-xs font-bold">Edit</button>{!role.is_system ? <button type="button" onClick={() => void removeRole(role)} className="rounded-xl border border-red-200 px-3 py-2 text-xs font-bold text-red-600">Remove</button> : null}</div></div><div className="mt-3 flex flex-wrap gap-1.5">{inventoryPermissionList.filter((item) => Boolean(inv[item.key])).map((item) => <span key={`inv-${item.key}`} className="rounded-lg bg-[var(--school-primary-soft)] px-2 py-1 text-[9px] font-semibold theme-primary">Inventory: {item.label}</span>)}{itemSrPermissionList.filter((item) => Boolean(sr[item.key])).map((item) => <span key={`sr-${item.key}`} className="rounded-lg bg-[var(--school-primary-soft)] px-2 py-1 text-[9px] font-semibold theme-primary">Item SR: {item.label}</span>)}{legacyModules.filter((item) => Boolean(role.permissions?.[item.key])).map((item) => <span key={`legacy-${item.key}`} className="rounded-lg bg-[var(--school-primary-soft)] px-2 py-1 text-[9px] font-semibold theme-primary">{item.label}</span>)}</div></article>})}{!loading && !roles.length ? <p className="rounded-2xl border border-dashed border-[var(--school-border)] p-8 text-center text-sm text-[var(--school-muted)]">No roles found.</p> : null}</div></section>
    </div>
  </AdminPageShell>;
}
