      ...
      {tab === "items" ? (
        ...
        <section className="min-w-0 rounded-3xl border border-[var(--school-border)] bg-[var(--school-surface)] p-4 shadow-sm sm:p-6 md:p-7">
          ...
          <div className="mt-5 space-y-6">
            <section>
              <div className="mb-3 flex items-center justify-between gap-3"><div><h3 className="text-sm font-black uppercase tracking-[0.14em] theme-primary">Awaiting Approval</h3><p className="mt-1 text-xs text-[var(--school-muted)]">{pendingSrs.length} request{pendingSrs.length === 1 ? "" : "s"} waiting for admin action</p></div></div>
              <div className="overflow-x-auto rounded-2xl border border-[var(--school-border)]">
                <table className="min-w-[1080px] w-full border-collapse text-left text-xs">
                  <thead className="bg-[var(--school-primary-soft)]"><tr><th className="px-3 py-3 font-black theme-primary">SR</th><th className="px-3 py-3 font-black theme-primary">Requester</th><th className="px-3 py-3 font-black theme-primary">Department / Class</th><th className="px-3 py-3 font-black theme-primary">Items</th><th className="px-3 py-3 font-black theme-primary">Requested</th><th className="px-3 py-3 font-black theme-primary">Action</th></tr></thead>
                  <tbody>
                    {pendingSrs.map((sr) => <tr key={sr.id} className="border-t border-[var(--school-border)] align-top hover:bg-[var(--school-primary-soft)]/20"><td className="px-3 py-3"><button type="button" onClick={() => setSelectedSr(sr)} className="text-left font-black theme-primary hover:underline">{sr.sr_number}</button><p className="mt-1 text-[10px] text-[var(--school-muted)]">{new Date(sr.requested_at).toLocaleDateString("en-GB")}</p></td><td className="px-3 py-3"><p className="font-black">{sr.requester_name}</p><p className="mt-1 text-[10px] text-[var(--school-muted)]">{sr.requester_login_id}</p></td><td className="px-3 py-3">{sr.department || "-"}{sr.class_name ? <p className="mt-1 text-[10px] text-[var(--school-muted)]">Class {sr.class_name}</p> : null}</td><td className="px-3 py-3"><div className="space-y-2">{sr.items.map((item) => <div key={item.item_id}><p className="font-bold">{item.item_name}</p><p className="text-[10px] theme-primary">{item.item_code} · Req {item.requested_quantity} {item.unit}</p></div>)}</div></td><td className="px-3 py-3 whitespace-nowrap"><span className="rounded-full border border-[var(--school-border)] px-2.5 py-1 font-bold">{sr.items.reduce((sum, item) => sum + Number(item.requested_quantity || 0), 0)} units</span></td><td className="px-3 py-3"><div className="flex flex-wrap gap-2"><button disabled={saving} onClick={() => void processSr(sr, "approve")} className="rounded-lg px-3 py-2 font-black theme-primary-bg disabled:opacity-50">Approve</button><button disabled={saving} onClick={() => void processSr(sr, "reject")} className="rounded-lg border border-red-200 px-3 py-2 font-black text-red-600 disabled:opacity-50">Reject</button><button type="button" onClick={() => setSelectedSr(sr)} className="rounded-lg border border-[var(--school-primary-border)] px-3 py-2 font-black theme-primary">View</button></div></td></tr>)}
                    {!loading && !pendingSrs.length ? <tr><td colSpan={6} className="px-4 py-8 text-center text-sm text-[var(--school-muted)]">No requests waiting for approval.</td></tr> : null}
                  </tbody>
                </table>
              </div>
            </section>
            ...
            <section>
              <div className="mb-3 flex items-center justify-between gap-3"><div><h3 className="text-sm font-black uppercase tracking-[0.14em]">SR History</h3><p className="mt-1 text-xs text-[var(--school-muted)]">{historySrs.length} processed request{historySrs.length === 1 ? "" : "s"}</p></div></div>
              <div className="overflow-x-auto rounded-2xl border border-[var(--school-border)]">
                <table className="min-w-[980px] w-full border-collapse text-left text-xs">
                  <thead className="bg-[var(--school-background)]"><tr><th className="px-3 py-3 font-black">SR</th><th className="px-3 py-3 font-black">Requester</th><th className="px-3 py-3 font-black">Items</th><th className="px-3 py-3 font-black">Status</th><th className="px-3 py-3 font-black">Processed</th><th className="px-3 py-3 font-black">Action</th></tr></thead>
                  <tbody>
                    {historySrs.map((sr) => <tr key={sr.id} className="border-t border-[var(--school-border)] align-top hover:bg-[var(--school-background)]"><td className="px-3 py-3"><button type="button" onClick={() => setSelectedSr(sr)} className="text-left font-black theme-primary hover:underline">{sr.sr_number}</button><p className="mt-1 text-[10px] text-[var(--school-muted)]">{new Date(sr.requested_at).toLocaleDateString("en-GB")}</p></td><td className="px-3 py-3"><p className="font-black">{sr.requester_name}</p><p className="mt-1 text-[10px] text-[var(--school-muted)]">{sr.requester_login_id}</p></td><td className="px-3 py-3"><div className="space-y-1.5">{sr.items.map((item) => <p key={item.item_id}><span className="font-bold">{item.item_name}</span> <span className="text-[10px] text-[var(--school-muted)]">Req {item.requested_quantity} · Issued {item.issued_quantity}</span></p>)}</div></td><td className="px-3 py-3"><span className="inline-flex rounded-full border border-[var(--school-border)] px-2.5 py-1 font-bold capitalize">{sr.status.replace(/_/g, " ")}</span></td><td className="px-3 py-3 whitespace-nowrap">{sr.processed_at ? new Date(sr.processed_at).toLocaleString("en-GB", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit", hour12: false }) : "-"}</td><td className="px-3 py-3"><div className="flex flex-wrap gap-2"><button type="button" onClick={() => setSelectedSr(sr)} className="rounded-lg border border-[var(--school-primary-border)] px-3 py-2 font-black theme-primary">View</button>{sr.status === "approved" || sr.status === "partially_issued" ? <button disabled={saving} onClick={() => void processSr(sr, "issue")} className="rounded-lg px-3 py-2 font-black theme-primary-bg disabled:opacity-50">Issue</button> : null}</div></td></tr>)}
                    {!loading && !historySrs.length ? <tr><td colSpan={6} className="px-4 py-8 text-center text-sm text-[var(--school-muted)]">No SR history found.</td></tr> : null}
                  </tbody>
                </table>
              </div>
            </section>
