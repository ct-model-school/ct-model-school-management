...
type Member = {
  ...
  qualification_point: number | null;
  grade: string | null;
...
const defaults = (t: MemberType): Partial<FormState> =>
  t === "teacher"
    ? { role: "Teacher", designation: "Teacher", department: "Academic" }
    : t === "accounts"
      ? { role: "Accounts", designation: "Accounts", department: "Accounts", account_role: "Accounts Manager" }
      : t === "other"
        ? { role: "Other", designation: "", department: "" }
        : { role: "Staff", designation: "", department: "" };

const idPrefix = (t: MemberType) =>
  t === "staff" ? "STID00001" : t === "teacher" ? "TCID00001" : t === "accounts" ? "ACID00001" : "OTID00001";
...
export default function MembersPage() {
  ...
  function changeType(t: MemberType) {
    setType(t);
  ...
  function edit(m: Member) {
    setForm({
      ...
      subject: m.subject || "",
      account_role: m.account_role || "Accounts Manager",
      role_title: m.role_title || "",
      salary: m.salary == null ? "" : String(m.salary),
      qualification: m.qualification || "",
      qualification_point: m.qualification_point == null ? "" : String(m.qualification_point),
      grade: m.grade || "",
  ...
  async function save(e: FormEvent) {
    ...
    const { data, error } = await supabase.rpc("store_admin_save_member", {
      ...
      p_qualification_point: form.qualification_point === "" ? null : Number(form.qualification_point),
      p_grade: form.grade || null,
      p_institute_name: form.institute_name || null,
      p_salary: form.salary === "" ? null : Number(form.salary),
      ...
      p_joining_date: form.joining_date || null,
      p_photo_url: form.photo_url,
      p_details: form.details,
      p_access_role: form.role,
    });

    if (error) {
      setMessage(error.message);
    } else {
      setMessage(form.id ? "Member updated successfully." : `Member created: ${(data as { member_id: string }).member_id}`);
      setForm({ ...blank, type: form.type, ...defaults(form.type) });
      await load();
    }
    setSaving(false);
  }
  ...
  async function remove(m: Member) {
    ...
    if (error) setMessage(error.message);
      ...
      setMessage(`${m.member_id} removed.`);
      await load();
    }
  }
  ...
  return (
    <AdminPageShell
      ...
      title="Staff, Teachers, Accounts & Others"
      description="Admin-created internal members only. Parents, Students and Management Committee are excluded because they use their own registration/profile flows."
    >
      <div className="mb-5 grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">
        {(["staff", "teacher", "accounts", "other"] as MemberType[]).map((t) => (
          <button key={t} onClick={() => changeType(t)} className={`rounded-xl px-4 py-3 text-sm font-bold capitalize ${type === t ? "theme-primary-bg" : "border border-[var(--school-border)] bg-[var(--school-surface)]"}`}>
            {t}
          </button>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-[minmax(360px,.72fr)_minmax(0,1.28fr)]">
        <form onSubmit={save} className="rounded-3xl border border-[var(--school-border)] bg-[var(--school-surface)] p-4 shadow-sm sm:p-6 md:p-7">
          <div className="flex items-start justify-between gap-3">
            <div>
              <h2 className="text-xl font-black sm:text-2xl">{form.id ? "Edit" : "Add"} {type}</h2>
              <p className="mt-1 text-xs leading-5 text-[var(--school-muted)] sm:text-sm">ID is generated automatically: {idPrefix(type)} and increases for every new member.</p>
            </div>
            {form.id ? <button type="button" onClick={() => setForm({ ...blank, type, ...defaults(type) })} className="shrink-0 rounded-xl border border-[var(--school-border)] px-3 py-2 text-xs font-bold">Cancel</button> : null}
          </div>

          <div className="mt-5 grid grid-cols-1 gap-3.5 sm:grid-cols-2 sm:gap-4">
            <label className="sm:col-span-2"><span className="label">Full Name *</span><input className="field w-full" value={form.full_name} onChange={(e) => set(set as never, e.target.value)} required /></label>
            ...
            {(type === "teacher" || type === "accounts") ? <>
              <label><span className="label">Qualification</span><input className="field w-full" value={form.qualification} onChange={(e) => set("qualification", e.target.value)} placeholder="B.Sc / M.A / MBA" /></label>
            ...
            <label><span className="label">NID</span><input className="field w-full" value={form.nid} onChange={(e) => set("nid", e.target.value)} /></label>
            ...
            <label><span className="label">Photo URL</span><input className="field w-full" value={form.photo_url} onChange={(e) => set("photo_url", e.target.value)} /></label>
        ...
        <section className="min-w-0 rounded-3xl border border-[var(--school-border)] bg-[var(--school-surface)] p-4 shadow-sm sm:p-6 md:p-7">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div><h2 className="text-xl font-black capitalize sm:text-2xl">{type} Members</h2><p className="mt-1 text-xs leading-5 text-[var(--school-muted)] sm:text-sm">Admin-only member records.</p></div>
            <div className="flex w-full gap-2 sm:w-auto"><input className="field min-w-0 flex-1 sm:w-64" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search ID / name" /><button onClick={() => void load()} className="rounded-xl px-4 py-3 text-sm font-bold theme-primary-bg">Search</button></div>
          </div>

          <div className="mt-5 space-y-3">
            {loading ? <p className="rounded-2xl border border-dashed border-[var(--school-border)] p-8 text-center text-sm text-[var(--school-muted)]">Loading...</p> : null}
            {visible.map((m) => (
              <article key={m.id} className="rounded-2xl border border-[var(--school-border)] p-4">
                <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                  <div className="min-w-0">
                    <p className="text-xs font-black theme-primary">{m.member_id}</p>
                    <h3 className="mt-1 break-words font-black">{m.full_name}</h3>
                    <p className="break-words text-xs text-[var(--school-muted)]">{m.role}{m.designation ? ` • ${m.designation}` : ""}{m.department ? ` • ${m.department}` : ""}{m.subject ? ` • ${m.subject}` : ""}</p>
                    {(m.salary != null || m.qualification || m.grade || m.institute_name) ? <p className="mt-2 break-words text-xs text-[var(--school-muted)]">{m.salary != null ? `Salary: ${m.salary}` : ""}{m.qualification ? ` • ${m.qualification}` : ""}{m.grade ? ` • ${m.grade}` : ""}{m.institute_name ? ` • ${m.institute_name}` : ""}</p> : null}
                  </div>
                  <div className="flex gap-2"><button onClick={() => edit(m)} className="rounded-xl border border-[var(--school-border)] px-3 py-2 text-xs font-bold">Edit</button><button onClick={() => void remove(m)} className="rounded-xl border border-red-200 px-3 py-2 text-xs font-bold text-red-600">Remove</button></div>
                </div>
              </article>
            ))}
            {!loading && !visible.length ? <p className="rounded-2xl border border-dashed border-[var(--school-border)] p-8 text-center text-sm text-[var(--school-muted)]">No members found.</p> : null}
          </div>
        </section>
      </div>
    </AdminPageShell>
