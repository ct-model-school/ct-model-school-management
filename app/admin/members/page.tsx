...
export default function MembersPage() {
  ...
  async function uploadPhoto(file: File) {
    ...
    try {
      ...
      const safeMember = form.id || crypto.randomUUID();
      const filePath = `${type}/${safeMember}-${Date.now()}.${extension}`;
      const { data, error } = await supabase.storage.from(photoBucket).upload(filePath, file, {
        cacheControl: "3600",
      ...
      setForm((current) => ({ ...current, photo_url: publicData.publicUrl }));
  ...
  const visible = members.filter((m) => {
    ...
    return !q || [m.member_id, m.full_name, m.role, m.designation, m.department, m.qualification, m.institute_name, m.email, m.phone]
  ...
  return (
    <AdminPageShell eyebrow="Community Access" title="Staff, Teachers, Accounts & Others" description="Admin-created internal members only. Parents, Students and Management Committee are excluded because they use their own registration/profile flows.">
      <div className="mb-5 grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">
        {(["staff", "teacher", "accounts", "other"] as MemberType[]).map((t) => <button key={t} onClick={() => changeType(t)} className={`rounded-xl px-4 py-3 text-sm font-bold capitalize ${type === t ? "theme-primary-bg" : "border border-[var(--school-border)] bg-[var(--school-surface)]"}`}>{t}</button>)}
      </div>
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-[minmax(300px,.7fr)_minmax(0,1.3fr)]">
        <form onSubmit={save} className="rounded-3xl border border-[var(--school-border)] bg-[var(--school-surface)] p-5 shadow-sm">
          <div className="flex items-start justify-between gap-3"><div><h2 className="text-xl font-black">{form.id ? "Edit" : "Add"} {type}</h2><p className="mt-1 text-xs leading-5 text-[var(--school-muted)]">ID is generated automatically: {idPrefix(type)} and increases for every new member.</p></div>{form.id ? <button type="button" onClick={() => setForm({ ...blank, type, ...defaults(type) })} className="shrink-0 rounded-xl border border-[var(--school-border)] px-3 py-2 text-xs font-bold">Cancel</button> : null}</div>
          <div className="mt-5 grid grid-cols-1 gap-3.5 sm:grid-cols-2">
            ...
            <label><span className="label">Email</span><input className="field w-full" type="email" value={form.email} onChange={(e) => set("email", e.target.value)} /></label><label><span className="label">Phone</span><input className="field w-full" value={form.phone} onChange={(e) => set("phone", e.target.value)} /></label><label><span className="label">WhatsApp</span><input className="field w-full" value={form.whatsapp} onChange={(e) => set("whatsapp", e.target.value)} /></label><label><span className="label">NID</span><input className="field w-full" value={form.nid} onChange={(e) => set("nid", e.target.value)} /></label><label><span className="label">Joining Date</span><input className="field w-full" type="date" value={form.joining_date} onChange={(e) => set("joining_date", e.target.value)} /></label>
            <div className="sm:col-span-2"><span className="label">Photo</span><div className="mt-1 flex items-center gap-3 rounded-xl border border-[var(--school-border)] p-2">{form.photo_url ? <img src={form.photo_url} alt="Member" className="h-14 w-14 rounded-lg object-cover" /> : <div className="flex h-14 w-14 items-center justify-center rounded-lg bg-[var(--school-primary-soft)] text-[10px] font-bold theme-primary">No Photo</div>}<div className="min-w-0 flex-1"><label className="inline-flex cursor-pointer rounded-lg border border-[var(--school-border)] px-3 py-2 text-xs font-bold">{uploadingPhoto ? "Uploading..." : "Choose Photo"}<input type="file" accept="image/*" className="hidden" disabled={uploadingPhoto} onChange={(e) => { const file = e.target.files?.[0]; if (file) void uploadPhoto(file); e.currentTarget.value = ""; }} /></label><p className="mt-1 text-[10px] text-[var(--school-muted)]">Max 2 MB. Saved automatically in {type}/ folder.</p></div></div></div>
            <label className="sm:col-span-2"><span className="label">Address</span><textarea className="field min-h-24 w-full py-3" value={form.address} onChange={(e) => set("address", e.target.value)} /></label>
