...
type FormState = {
  ...
  nid: string; address: string; joining_date: string; photo_url: string; details: string;
};
...
const blank: FormState = {
  ...
  institute_name: "", email: "", phone: "", whatsapp: "", nid: "", address: "", joining_date: "", photo_url: "", details: "",
};
...
export default function MembersPage() {
  ...
  async function loadDocuments(memberId: string) {
    ...
    const cert = docs.find((d) => d.document_type === "qualification_certificate");
    const front = docs.find((d) => d.document_type === "nid_front");
    const back = docs.find((d) => d.document_type === "nid_back");
    return { cert, front, back };
  }
  ...
  async function edit(m: Member) {
    setForm({
      ...
      whatsapp: m.whatsapp || "", nid: m.nid || "", address: m.address || "", joining_date: m.joining_date || "",
      photo_url: m.photo_url || "", details: m.details || "",
    });
  ...
  async function uploadDocument(file: File, memberId: string, documentType: DocumentType) {
    ...
    await supabase.from("member_documents").upsert({ member_id: memberId, member_type: type, document_type: documentType, file_url: publicData.publicUrl, storage_path: data.path }, { onConflict: "member_id,document_type" });
  }
  ...
  async function save(e: FormEvent) {
    ...
    try {
      ...
      const { data, error } = await supabase.rpc("store_admin_save_member", {
        ...
        p_whatsapp: form.whatsapp, p_nid: form.nid, p_address: form.address, p_joining_date: form.joining_date || null,
      ...
      if (profileFile) {
        ...
        await supabase.rpc("store_admin_save_member", {
          ...
          p_address: form.address, p_joining_date: form.joining_date || null, p_photo_url: publicData.publicUrl, p_details: form.details, p_access_role: form.role,
        });
      }
