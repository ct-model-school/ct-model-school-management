...
export default async function AdminPage() {
  ...
  const modules = [
    {
      ...
      title: "Inventory",
      description: "Manage items, stock, item information and Inventory-side SR approval and processing.",
    },
    {
      href: "/admin/item-sr",
      title: "Item SR",
      description: "Handle member Item Service Requests, including request creation, search and own SR history.",
    },
  ];
  ...
  return (
    <div className="mx-auto max-w-7xl">
      ...
      <section className="mt-6 grid gap-5 md:grid-cols-2" aria-label="Ready administration modules">
        {modules.map((module) => (
          <Link
            key={module.href}
            href={module.href}
            className="group block rounded-3xl border border-[var(--school-border)] bg-[var(--school-surface)] p-6 shadow-sm transition hover:-translate-y-0.5 hover:border-[var(--school-primary-border)] hover:shadow-md md:p-8"
          >
      ...
      <section className="mt-6 rounded-2xl border border-dashed border-[var(--school-border)] bg-[var(--school-primary-soft)] p-5 md:p-6">
        <p className="text-[10px] font-black uppercase tracking-[0.14em] theme-primary">Module Rollout</p>
        <h2 className="mt-1 text-base font-black">More modules will appear here as they become ready.</h2>
        <p className="mt-1 text-sm text-[var(--school-muted)]">Existing database structures and unfinished modules are kept intact. Only ready modules are exposed in the Admin UI.</p>
      </section>
    </div>
  );
}
