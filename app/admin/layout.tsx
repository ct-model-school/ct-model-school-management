...
export default async function AdminLayout({children}:{children:React.ReactNode}){
  ...
  const visiblePrimary=primaryNavigation.filter(item=>{if(item.adminOnly)return adminOnly;if(item.permission==="inventory_view")return hasInventory;if(item.permission==="procurement")return hasProcurement;if(item.permission==="accounts_access")return hasAccounts;if(item.permission==="item_sr")return hasItemSr;return allow(item.permission||"")});
