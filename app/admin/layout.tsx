...
export default async function AdminLayout({children}:{children:React.ReactNode}){
  ...
  const has=(key:string)=>adminOnly||Boolean(permissions[key]);
  const hasInventory=has("inventory")||has("procurement");
  const hasAccounts=has("accounts")||has("procurement");
  const hasItemSr=has("item_sr");
  ...
  const visiblePrimary=primaryNavigation.filter(item=>item.adminOnly?adminOnly:Boolean(item.permission&&has(item.permission)));
